...
private:
  ...
  // 内部缓冲区配置
  static const uint16_t MAX_PACKET_SIZE = 128; // 最大数据包大小
  static const uint16_t BUFFER_SIZE = MAX_PACKET_SIZE + sizeof(Header);

  ParserState state_ = STATE_PREAMBLE_1;
  uint8_t expected_preamble_1_; // 期望的前导字节1
  uint8_t expected_preamble_2_; // 期望的前导字节2

  uint8_t send_seq_id_ = 0;

  uint8_t last_seq_id_ = 0;
  uint32_t seq_id_right_ = 0;
  uint32_t seq_id_err_ = 0;
  ...
  PackerBuf packer_buf_[4] = {0}; //用来存放连包
...
public:
  // 构造函数
  PacketParser(uint8_t preamble1 = 0xAA, uint8_t preamble2 = 0x55)
      : expected_preamble_1_(preamble1), expected_preamble_2_(preamble2) {}
  ...
  void setPreamble(uint8_t preamble1, uint8_t preamble2) {
    expected_preamble_1_ = preamble1;
    expected_preamble_2_ = preamble2;
  }
  ...
  //     if (byte == expected_preamble_1_) {
  //       state_ = STATE_PREAMBLE_2;
  //       // 存储第一个前导字节到header
  //       header_.preamble_1 = byte;
  //     }
  //     break;

  //   case STATE_PREAMBLE_2:
  //     if (byte == expected_preamble_2_) {
  //       state_ = STATE_SEQ_ID;
  //       header_.preamble_2 = byte;
  //     } else {
  //       reset(); // 不是预期的前导字节，重置
  //     }
  //     break;
  ...
  //     header_.reserve = byte; // 先存储低字节
  //     state_ = STATE_RESERVE_HIGH;
  //     break;

  //   case STATE_RESERVE_HIGH:
  //     header_.reserve |= (byte << 8); // 更新完整的保留字段
  ...
  //   case STATE_PAYLOAD:
  //     // 存储数据载荷
  //     if (data_buffer_cur_len_ < BUFFER_SIZE) {
  //       data_buffer_[data_buffer_cur_len_++] = byte;
  //       data_buffer_cur_len_++;
    ...
    for (uint16_t i = 0; i < received; i++) {
      ...
      case STATE_PAYLOAD: {
        ...
        if (data_buffer_cur_len_ == 0 && remaining_buf >= remaining_data) {
          ...
          // 计算实际可以拷贝的数据量
          uint32_t copy_size =
              (remaining_buf < remaining_data) ? remaining_buf : remaining_data;

          // 将数据拷贝到data_buffer
          memcpy(data_buffer_ + data_buffer_cur_len_, recv_buf + i, copy_size);
  ...
  //     case STATE_RESERVE_LOW:
  //       header_.reserve = recv_buf[i]; // 先存储低字节
  //       state_ = STATE_RESERVE_HIGH;
  //       break;

  //     case STATE_RESERVE_HIGH:
  //       header_.reserve |= (recv_buf[i] << 8); // 更新完整的保留字段
  ...
  //           // CRC校验成功
  //           if (packet_callback_) {
  //             packet_callback_(&header_, nullptr, 0);
  //           }
  //           reset();
  //           is_unpack_success = 1;
  //         } else {
  ...
  //         data_buffer_[data_buffer_index++] = recv_buf[i];
  //         data_buffer_cur_len_++;
  ...
  Header buildPacket2(const uint8_t *data, uint16_t data_length) {
    ...
    // 设置前导字节（使用默认值或类成员变量）
    packer_header.preamble_1 = expected_preamble_1_; // 使用类中的前导字节配置
    packer_header.preamble_2 = expected_preamble_2_;
    // 设置序列号并递增
  ...
  // 判断缓冲区是否已满
  bool isBufferFull() {
    return ((packer_buf_end_ + 1) % 4) == packer_buf_start_;
  }
  ...
  uint8_t getBufferCount() {
    if (packer_buf_end_ >= packer_buf_start_) {
      ...
      return 4 - (packer_buf_start_ - packer_buf_end_);
    }
  }
  ...
  int32_t write_buf(const uint8_t *data, uint32_t len) {
    ...
    // 更新缓冲区结束指针
    packer_buf_end_ = (packer_buf_end_ + 1) % 4;
  ...
  int32_t read_buf(uint8_t *data, uint32_t len) {
    ...
    // 更新缓冲区开始指针
    packer_buf_start_ = (packer_buf_start_ + 1) % 4;
