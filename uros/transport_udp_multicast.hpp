...
inline void TransportUdpMulticast::initSocket(const char *mcast_ip,
  ...
  // 设置组播回环（同机测试时需要）
  u_char loop = 1;
  res = setsockopt(sendfd_, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop));
  CHECK(res >= 0)
...
inline int TransportUdpMulticast::send(const void *data, size_t len, int prio,
                                       int timeout_ms) {
...
inline int TransportUdpMulticast::recv(void *data, size_t len, int *prio,
                                       int timeout_ms) {
...
inline TransportUdpMulticast::~TransportUdpMulticast() {
  if (sendfd_ >= 0) {
    close(sendfd_);
  }
  if (recvfd_ >= 0) {
