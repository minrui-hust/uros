...
bool ClientT<TReq, TRsp>::call(const TReq &req, TRsp &rsp, int timeout_ms) {
  ...
  req.__meta__.sys = System::Id();
  // req.__meta__.id.req.sys_from; // filled by transport
  // req.__meta__.id.req.sys_to; // filled by transport
  req.__meta__.id.req.service = service_->id();
  req.__meta__.id.req.client = this->id();
  req.__meta__.id.req.seq = seq_++;
